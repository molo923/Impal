...
@section('content')
    ...
    <div class="container-fluid">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <a href="{{ route('banksampah.pegawai-create') }}" class="btn btn-primary ml-auto"><i class="mdi mdi-plus"></i> Tambah</a>
                        </div>
                                ...
                                <tbody>
                                    @foreach($pegawais as $pegawai)
                                        <tr>
                                            ...
                                            <td>{{ $pegawai->name }}</td>
                                            <td>{{ $pegawai->user->phone_number }}</td>
                                            <td class="font-bold">{{ ucfirst($pegawai->type) }}</td>
                                            <td>{{ $pegawai->user->email }}</td>
                                            <td>{{ $pegawai->gender }}</td>
                                            <td><span class="badge badge-{{ $pegawai->user->status_id == config('constants.statuses.AKTIF') ? 'success' : 'danger' }}">{{ $pegawai->user->status->name }}</span></td>
                                            <td>
                                                <form id="toggleStatusForm{{ $pegawai->id }}" class="d-none" action="{{ route('pegawai.toggle-status', $pegawai->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                </form>
                                                <button class="btn btn-primary btn-sm" data-pegawai-type="{{ $pegawai->type }}" data-id="{{ $pegawai->id }}" data-toggle="modal" data-target="#editPegawaiModal">
                                                    Kelola
                                                </button>
                                                <button class="btn btn-{{ $pegawai->user->status_id == config('constants.statuses.AKTIF') ? 'secondary' : 'primary' }} btn-sm" onclick="event.preventDefault();
                                                                                                                                                                                        document.getElementById('toggleStatusForm{{ $pegawai->id }}').submit()">
                                                    {{ $pegawai->user->status_id == config('constants.statuses.AKTIF')
                                                            ? 'Non-aktif'
                                                            : 'Aktif' }}
                                                </button>
                                            </td>
        ...
        <div class="modal fade" id="addPegawaiModal" style="overflow-y:auto;">
        ...
        <div id="editPegawaiModal" class="modal fade">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Kelola Pegawai</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form id="editPegawaiForm" action="" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="form-group">
                                <label for="type">Posisi Saat Ini</label>
                                <p class="m-0 font-bold"></p>
                            </div>
                            <div class="form-group">
                                <label for="type">Tipe Pegawai</label>
                                <select name="type" placeholder="Pilih jenis pegawai..." class="select2 form-control @error('type') is-invalid @enderror custom-select" style="width: 100%; height:36px;">
                                    <option></option>
                        ...
                        <p class="text-muted m-0">(*) Pilih posisi untuk melanjutkan.</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" action="submit" disabled="disabled" class="btn btn-primary" onclick="event.preventDefault();
                                                                                document.getElementById('editPegawaiForm').submit();">Tambah</button>
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                    </div>
                </div>
            </div>
                    ...
                    success: function (data) {
                        if (data.code !== 404) {
                            ...
                            selectCity.select2({
                                ...
                                width: "100%",
                                data: the_data,
                                dropdownParent: $(".modal-body","#addPegawaiModal"),
                            });
                            selectCity.trigger({
                                type: "select2:select",
                                params: {
